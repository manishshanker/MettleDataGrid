...
window.mettleDataGridFakeSource = {};

(function ($, mettleDataGridFakeSource) {
    "use strict";

    mettleDataGridFakeSource.fakeData = {};
    mettleDataGridFakeSource.FakeLocalSource = function (dataset) {
        ...
        function callAction(actionType, requestData, callback) {
            ...
            var fakeDataStoreRows = MettleDataGrid.DataStore.utils.getFilteredRows($.extend(true, [], dataset.rows), stateData.filterBy, fakeDataStoreColumns, dataset.filters, true);
            if (hasGroupByOrSortBy(stateData)) {
                MettleDataGrid.DataStore.utils.sortData(stateData, fakeDataStoreColumns, fakeDataStoreRows, true);
            }
            var dummyResponseData = {};
            var pageSize = (requestData.pageSize || dataset.pageSize || 25);
            if (actionType === "fetchRows") {
                dummyResponseData = {
                    rows: MettleDataGrid.DataStore.utils.getRows(stateData.pageOffset - 1, pageSize, fakeDataStoreRows)
                };
            } else {
        ...
        function getDummyDataForFirstRequest(pageSize, fakeDataStoreColumns, fakeDataStoreRows) {
            return {
                ...
                rows: MettleDataGrid.DataStore.utils.getRows(0, pageSize, fakeDataStoreRows),
                state: {
                    groupBy: [],
                    sortBy: []
                },
    ...
    mettleDataGridFakeSource.getFilterData = function (columnIdentifier, rows) {
        var filterData = [];

(function ($) {
    ...
    $.mettleDataGrid.filter.MULTI_SELECT = {
        ...
        init: function ($filter, onFilter, gridId, $mettleDataGrid) {
            if ($.fn.multiselect) {
                $filter.multiselect({
                    overrideWidth: "100%",
            ...
            $mettleDataGrid.find(".mettleDataGrid-rows").bind("scroll.multiselect-" + $filter.attr("id"), function () {
        ...
        destroy: function ($filter, $mettleDataGrid) {
            if ($.fn.multiselect) {
                $filter.multiselect("destroy");
            }
            $mettleDataGrid.find(".mettleDataGrid-rows").unbind("scroll.multiselect-" + $filter.attr("id"));
        }
    };
